...
$| = 1;
my $word1 = shift;
if (!defined($word1) || $word1 eq "")
{
        die "need a word to search for";
}
my $word2 = shift;
if (!defined($word2) || $word2 eq "")
{
        die "need a second word to search for";
}
my $word3 = shift;
if (!defined($word3) || $word3 eq "")
{
        die "need a third word to search for";
}
mkdir "FGREP";
my $output = "FGREP/$word1" . "_" . "$word2" . "_" . "$word3.txt";
if (-e $output)
{
}
else
{
        ...
        foreach my $file (sort @files)
        {
                ...
                if ($file_open)
                {
                        ...
                        close(FILE);
                        if ($line =~ /$word1/i && $line =~ /$word2/i && $line =~ /$word3/i)
                        {
                                print OUTPUT
"---------------------------------------------------------------------------\n";
                                print OUTPUT
