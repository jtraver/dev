...
{
    my ($filename) = @_;
    print "nfl100: filename = $filename\n";
    my $fileopen = open(NFL, $filename);
    if (!$fileopen)
    {
    ...
    foreach my $line (@lines)
    {
        ...
        if ($line =~ /^(\d+)\t([^,]+), ([^,]+), (.*)$/)
        {
            ...
            my $team = $4;
            print "$line\n";
            print "  $rank $name $position $team\n";
            $team_count{$team}++;
            $teams{$team}{$rank}{$name}{$position}++;
        }
    }
    print "\ncount by team\n";
    my $teams = 0;
    foreach my $team (reverse sort {$team_count{$a} <=> $team_count{$b}} keys %team_count)
    {
        $teams++;
        my $count = $team_count{$team};
        print "$teams $count $team\n";
        my $rankref = $teams{$team};
        my %rankhash = %$rankref;
        foreach my $rank (sort {$a <=> $b} keys %rankhash)
        {
            # print "  $rank\n";
            my $nameref = $rankhash{$rank};
            my %namehash = %$nameref;
            my @name = keys %namehash;
            my $name = $name[0];
            # print "    $name\n";
            my $posref = $rankhash{$rank}{$name};
            my %poshash = %$posref;
            my @pos = keys %poshash;
            my $pos = $pos[0];
            print "  $rank, $name, $pos, $team\n";
        }
    }
}
